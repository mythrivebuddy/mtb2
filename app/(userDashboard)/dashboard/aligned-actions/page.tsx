    ...
    <div className="container mx-auto py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Schedule and track your 1% Start actions</p>
        </div>
        <Button
      ...
      {hasCreatedToday && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-blue-800 text-sm flex items-center">
            ...
            You can create only one 1% Start action per day to help you stay focused on what matters most.
          </p>
        </div>
      )}
      ...
      {isLoading ? (
            ...
            <Card key={action.id}>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle className="flex items-center gap-2">
                    ...
                    <span>
                      {format(new Date(action.timeFrom), "h:mm a")} - {format(new Date(action.timeTo), "h:mm a")}
                    </span>
                  </CardTitle>
              ...
              <CardContent>
                <div className="mb-4">
                  ...
                  <p className="p-3 bg-gray-50 rounded-md">{action.selectedTask}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">Other Tasks:</h3>
                  <ul className="space-y-2">
                    {action.tasks.map((task: string, i: number) => (
                      task !== action.selectedTask && (
                        <li key={i} className="p-3 bg-gray-50 rounded-md">{task}</li>
                      )
                    ))}
                  </ul>
                </div>
                  ...
                  {action.completed ? (
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-sm text-gray-500">
                        You will receive a reminder 5 minutes before your scheduled time.
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <h3 className="text-xl font-medium mb-2">No 1% Start Action Created Today</h3>
          <p className="text-gray-500 mb-4">
            Create an 1% Start actions to focus on what matters most to you today.
          </p>
          <Button 
            onClick={() => setShowWizard(true)}
            className="bg-jp-orange hover:bg-jp-orange/90"
