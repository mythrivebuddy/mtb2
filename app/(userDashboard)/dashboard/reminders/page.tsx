...
const addReminder = async (
    newReminder: Omit<
        Reminder,
        "id" | "isActive" | "createdAt" | "lastNotifiedAt"
    >,
): Promise<Reminder> => {
...
const updateReminder = async (
    updatedReminder: Partial<Reminder> & { id: string },
): Promise<Reminder> => {
...
const ReminderForm = ({
    ...
    onSave: (
        data: Omit<Reminder, "id" | "isActive" | "createdAt" | "lastNotifiedAt">,
    ) => void;
    onClose: () => void;
    isEditMode?: boolean;
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {defaultImages.map((imgSrc) => (
                            <div
                                ...
                                className={`relative rounded-lg overflow-hidden cursor-pointer aspect-square transition-all duration-200 ${selectedImage === imgSrc ? "ring-4 ring-blue-500" : "ring-2 ring-transparent hover:ring-blue-300"}`}
                            >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                    src={imgSrc}
                                    alt="Reminder image option"
            ...
            >
                ...
                <div className="space-y-3">
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-6 py-3 text-lg font-semibold text-white bg-green-500 rounded-xl shadow-md hover:bg-green-600 transition-transform hover:scale-105"
                    >
                        <Check size={22} /> Done
                    </button>
                    <button
                        ...
                        disabled={isSnoozing}
                        className="w-full flex items-center justify-center gap-2 px-6 py-3 text-lg font-semibold text-gray-700 bg-gray-200 rounded-xl shadow-md hover:bg-gray-300 transition-transform hover:scale-105 disabled:bg-gray-300"
                    >
                        {isSnoozing ? (
...
const ReminderCard = ({
    ...
    onViewClick: (reminder: Reminder) => void;
}) => {
    // --- FIX: Logic to calculate if a reminder is currently snoozed from DB data ---
    const isSnoozed =
        reminder.lastNotifiedAt &&
        new Date(reminder.lastNotifiedAt).getTime() + 15 * 60 * 1000 > Date.now();
    ...
    return (
        ...
        >
            ...
            <img
                src={reminder.image}
                alt={reminder.title}
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-lg object-cover flex-shrink-0 ml-4"
                onError={(e) => {
                    e.currentTarget.src =
            ...
            {isEditModalOpen && currentReminder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
                    <ReminderForm
                        ...
                        onSave={(data) =>
                            updateMutation.mutate({ ...data, id: currentReminder.id })
                        }
                        ...
                        {!isLoading &&
                            ...
                            reminders.map((reminder) => (
                                <ReminderCard
                                    key={reminder.id}
                                    reminder={reminder}
                                    onEditClick={(r) => handleOpenModal(r, "edit")}
                                    onViewClick={(r) => handleOpenModal(r, "view")}
                                />
                            ))}
                    </div>
                </div>
