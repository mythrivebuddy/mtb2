...
type BuddyLensRequest = Prisma.BuddyLensRequestGetPayload<{ include: { reviewer: true } }> & {
  feedbackType: string;
}
...
type Review = Prisma.BuddyLensReviewGetPayload<{ include: { request: true, reviewer: true } }>;
  ...
  useEffect(() => {
    if (selectedRequest) {
      ...
      const calculateRemaining = () => {
        const elapsed = Math.floor((Date.now() - parseInt(startTime as string)) / 1000);
        ...
        {isRequestsLoading || isNotificationsLoading || isReviewedRequestsLoading || isSelectedRequestLoading ? (
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      ...
                      Time Limit: {
                        req.tier === 'PREMIUM' ? '10 minutes' : 
                        req.tier === 'ENTERPRISE' ? '15 minutes' : 
                        '5 minutes'
                      }
                    </p>
                    <a
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-gray-100 rounded-lg">
              ...
              <div className={`font-mono font-bold text-lg ${isExpired ? 'text-red-600' : timeRemaining && timeRemaining < 60 ? 'text-orange-600' : 'text-blue-600'}`}>
                {formatTimeRemaining()}
              </div>
            </div>
            ...
            {isExpired && (
              <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                ...
                <p>You can no longer submit this review. Please claim a new request.</p>
              </div>
            )}
            ...
            >
              {submitReviewMutation.isPending ? 'Submitting...' : isExpired ? 'Time Expired' : 'Submit Review'}
            </Button>
          </div>
        )}
                ...
                <Card key={review.id} className="p-4">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        <span className="font-medium text-black">Reviewer Name:</span> {review.reviewer.name}
                      </p>
