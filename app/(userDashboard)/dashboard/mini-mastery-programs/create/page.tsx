  ...
  const fetchPrograms = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/mini-mastery-programs?${params.toString()}`,
      );
      if (!res.ok) {
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 md:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-50">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              {statusFilter !== "ALL" && (
                <div
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold ${STATUS_CONFIG[statusFilter].color}`}
                >
              ...
              <tbody className="divide-y divide-gray-50">
                ...
                {!loading &&
                  programs.map((program) => {
                    const statusKey = (program.status ??
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {program.status === "DRAFT" ? (
                              program.isComplete ? (
                                <button
                                  onClick={() =>
                                    handleSubmitForReview(program.id)
                                  }
