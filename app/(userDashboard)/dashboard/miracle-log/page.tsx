...
async function getMiracleLogDailyLimit() {
  ...
  return typeof featureResult.config === "object"
...
export default async function MiracleLogPage() {
  const [logs, streak, dailyLimit] = await Promise.all([
    getMiracleLogs(),
    getStreak(),
  ...
  return (
    ...
    >
      <MiracleLogClient initialLogs={logs} initialStreak={streak} dailyLimit={dailyLimit}/>
    </Suspense>
  );
}
