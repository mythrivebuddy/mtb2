...
const CheckoutContent = () => {
  ...
  // ✅ Changed: selectedCurrency is now REQUIRED for mixed currency carts
  const [selectedCurrency, setSelectedCurrency] = useState<"INR" | "USD" | null>(null);
  ...
  // ✅ Updated: Place order mutation now includes currency
  const placeOrderMutation = useMutation({
    mutationFn: async () => {
      if (!selectedCurrency) {
      ...
      await axios.post("/api/user/store/items/place-order", {
        items: parsedCartItems.map(({ itemId, quantity }) => ({ itemId, quantity })),
        currency: selectedCurrency, // ✅ Send selected currency to backend
      });
    },
    onSuccess: () => {
      toast.success("Order placed successfully!");
  ...
  // ─── Currency derivation (must be before early returns) ────────────────────
  const itemCurrencies = items?.map((item) => item.currency || "INR") || [];
  const uniqueCurrencies = [...new Set(itemCurrencies)];
  const isMixedCurrency = uniqueCurrencies.length > 1;

  // ✅ Auto-select currency if there's only one (must be before early returns)
  React.useEffect(() => {
    if (items && !isMixedCurrency && uniqueCurrencies.length === 1 && !selectedCurrency) {
      setSelectedCurrency(uniqueCurrencies[0] as "INR" | "USD");
    }
  ...
  // ✅ Calculate single total (when currency is selected)
  const calculateSingleTotal = () => {
    ...
    const total = items.reduce((sum, item, i) => {
      const itemCurrency = item.currency || "INR";
      const rawPrice = item.basePrice * parsedCartItems[i].quantity;
      return sum + convertPrice(rawPrice, itemCurrency, selectedCurrency);
    }, 0);
    
    return { currency: selectedCurrency, total };
  };

  const totalsByCurrency = calculateTotalsByCurrency();
  const singleTotal = calculateSingleTotal();
  
  // ✅ Updated: Can only proceed if currency is selected AND billing is complete
  const canProceed = hasBilling && !isEditingBilling && selectedCurrency !== null;
      ...
      <div className="container mx-auto p-4 md:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* 3. Currency Selection (MANDATORY for mixed currencies) */}
            {isMixedCurrency && (
              <div className="bg-gradient-to-br from-orange-50 to-red-50 p-6 rounded-xl shadow-md border-2 border-orange-300">
                ...
                <p className="text-sm text-gray-900 mb-4 font-bold">
                  ⚠️ Select the currency you want to pay in:
                </p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {uniqueCurrencies.map((currency) => (
                    <button
                      ...
                      className={`p-5 rounded-xl border-3 transition-all duration-200 ${
                        selectedCurrency === currency
                          ? "border-green-600 bg-green-600 text-white shadow-xl transform scale-105 ring-4 ring-green-200"
                    ...
                    >
                      <div className="text-center">
                        <div className="flex items-center justify-center gap-2 text-3xl font-bold mb-2">
                          {currency === "INR" ? (
                            <RupeeIcon className={`w-7 h-7 ${selectedCurrency === currency ? 'text-white' : 'text-orange-600'}`} />
                          ) : (
                            <DollarIcon className={`w-7 h-7 ${selectedCurrency === currency ? 'text-white' : 'text-green-600'}`} />
                          )}
                          {currency}
                        </div>
                        {selectedCurrency === currency ? (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-bold text-green-900 mb-1">
                          Payment Currency Selected: {selectedCurrency}
                        </p>
                        <p className="text-xs text-green-800">
                          All items will be converted to {selectedCurrency} at checkout. 
                          Exchange rate: 1 USD = ₹83.50
                        </p>
                        <button
                          onClick={() => setSelectedCurrency(null)}
                          className="mt-2 text-xs font-semibold text-green-700 hover:text-green-900 underline"
                        >
                          Change Currency
                        </button>
                      </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-red-800">
                          You must select a payment currency before placing your order.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md">
              ...
              {/* Conversion notice */}
              {selectedCurrency && isMixedCurrency && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-bold text-green-900 mb-1">
                        Paying in {selectedCurrency}
                      </p>
                      <p className="text-sm text-green-800">
                        All prices converted using rate: 1 USD = ₹83.50
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-lg sticky top-4">
              ...
              <div className="space-y-4 mb-5">
                {/* Show single total when currency is selected */}
                {singleTotal ? (
                  <>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-700 font-medium">
                  ...
                  // Multiple currencies without selection
                  <>
                    <div className="space-y-2">
                      {Object.entries(totalsByCurrency).map(([currency, total]) => {
                        const count = itemCurrencies.filter(ic => ic === currency).length;
              ...
              <div className="space-y-4">
                ...
                {/* ✅ New warning for currency selection */}
                  ...
                  <div className="bg-red-50 border-2 border-red-400 rounded-lg p-3 animate-pulse">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm text-red-800 font-bold">
                        Select a payment currency above to place your order.
                      </p>
                    </div>
                  </div>
                )}
