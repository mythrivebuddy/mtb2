...
const CustomDropdown = ({
    ...
    const ringClass = isBlue ? "ring-blue-100 border-blue-300 dark:ring-blue-500/20 dark:border-blue-500" : "ring-violet-100 border-violet-300 dark:ring-violet-500/20 dark:border-violet-500";
    ...
    const hoverBg = isBlue ? "hover:bg-blue-50 dark:hover:bg-blue-500/10" : "hover:bg-violet-50 dark:hover:bg-violet-500/10";
    const selectedBg = isBlue ? "bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300" : "bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300";
    ...
    return (
        <div ref={ref} className="relative w-full sm:w-auto sm:min-w-[200px]">
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                className={`w-full flex items-center justify-between gap-3 px-4 py-3 bg-white dark:bg-slate-900 border-2 rounded-xl text-sm font-medium transition-all duration-200 focus:outline-none ${open
            ...
            >
                <span className={value ? "text-gray-800 dark:text-slate-100" : "text-gray-700 dark:text-slate-300"}>{selectedLabel}</span>
                <ChevronDown className={`w-4 h-4 shrink-0 transition-transform duration-200 ${open ? "rotate-180 text-gray-600 dark:text-slate-400" : "text-gray-400"}`} />
            </button>

            {open && (
                <div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
                        ...
                        {options.map((opt) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 text-left ${isSelected ? `${selectedBg} font-semibold` : `text-gray-700 dark:text-slate-300 ${hoverBg}`
                                        }`}
                                >
                                    <span>{opt.label}</span>
    ...
    const pageContent = (
        <>
            ...
            <div className="container mx-auto p-4 md:p-6 lg:p-8">
                ...
                {/* Header */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md dark:border dark:border-slate-700 p-6 mb-6">
                    <div className="flex justify-between items-center flex-wrap gap-4">
                        <div>
                            ...
                            <p className="text-gray-600 dark:text-slate-400 mt-1">Explore curated products that support your growth, clarity, and transformation.</p>
                        </div>
                        {
                            session?.user && (
                        ...
                        <div className="flex-1 min-w-[200px]">
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full px-5 py-3 pl-12 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-blue-400 focus:outline-none transition-all duration-200 text-gray-700 dark:text-gray-200 dark:bg-slate-950 dark:border-slate-700 dark:focus:border-blue-500 placeholder-gray-400 dark:placeholder-slate-500 text-sm"
                                />
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-gray-600 dark:text-slate-400 font-medium">{filteredItems.length} {filteredItems.length === 1 ? "product" : "products"} found</span>
                </div>

                {/* Grid */}
                {filteredItems.length === 0 ? (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md dark:border dark:border-slate-700 p-12 text-center">
                        <div className="max-w-md mx-auto">
                            ...
                            <h3 className="text-xl font-semibold text-gray-700 dark:text-slate-100 mb-2">No products found</h3>
                            <p className="text-gray-500 dark:text-slate-400">{searchQuery ? "Try adjusting your search or filters" : "No items available at the moment"}</p>
                        </div>
                    </div>
                ) : (
                    <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {filteredItems.map((item) => {
                            ...
                            return (
                                <div key={item.id} className="bg-white dark:bg-slate-900 rounded-2xl shadow-md hover:shadow-2xl dark:border dark:border-slate-700 dark:hover:shadow-black/40 transition-all duration-300 overflow-hidden group flex flex-col">
                                    <div className="relative w-full aspect-square bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-800 dark:to-slate-900 overflow-hidden">
                                        ...
                                        <button
                                            ...
                                            className="absolute top-3 right-3 p-2.5 bg-white/90 dark:bg-slate-950/90 backdrop-blur-sm rounded-full shadow-md hover:bg-white dark:hover:bg-slate-900 transition-all duration-300"
                                        >
                                            <Heart
                                                className={
                                                    wishlist.some((w) => w.itemId === item.id)
                                                        ? "text-red-500 w-5 h-5 fill-red-500 transition"
                                                        : "text-gray-600 dark:text-slate-400 w-5 h-5 transition"
                                                }
                                            />
                                        </button>
                                        <div className="absolute top-3 left-3">
                                            <span className="px-3 py-1.5 bg-white/90 dark:bg-slate-950/90 backdrop-blur-sm text-blue-600 dark:text-blue-300 text-xs font-semibold rounded-full shadow-md">{item.category.name}</span>
                                        </div>
                                    ...
                                    <div className="p-5 flex flex-col flex-1">
                                        <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-2 line-clamp-2 min-h-[3.5rem]">{item.name}</h3>
                                        <div className="flex items-center justify-between gap-2 mb-4">
                                            ...
                                            <Link
                                                href={`/profile/${item.createdByUserId}`}
                                                target="_blank"
                                                className="px-2 py-1 rounded-md text-xs font-medium text-gray-700 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-300 shadow-sm"
                                            >
                                                By {item.createdByRole === "ADMIN" ? "MTB" : item.creator?.name || "Coach"}
                                            </Link>
                                        </div>
