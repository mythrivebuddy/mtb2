...
    console.log(`GET /api/events :: Successfully fetched ${events.length} events for user ${session.user.id}.`);
...
    const dataToUpdate: { [key: string]: string | boolean | Date | null | undefined } = { ...updateData };
...
        dataToUpdate.end = dataToUpdate.end ? new Date(dataToUpdate.end as string) : null;
    }
