// /api/billing/one-time-plan-callback
...
export async function GET(req: Request) {
  try {
    ...
    if (!orderId || !purchaseId) {
      console.log("missing order id and purchase id");
    ...
    if (data.order_status === "PAID" || data.order_status === "COMPLETED") {
      ...
      return NextResponse.redirect(
        new URL(`/dashboard/membership/success?pid=${purchaseId}&type=membership`, req.url),
        303
      );
    }
    ...
    return NextResponse.redirect(
      new URL(`/dashboard/membership/failure?error=${data.order_status}`, req.url),
      303
    );
  } catch (err) {
    console.error("One Time Plan Callback Error:", err);
    return NextResponse.redirect(
