...
// console.log("")
  ...
  const plan = await tx.subscriptionPlan.findUnique({ // some timeout error here.
    where: { id: order.planId },
    select: { interval: true }
  });
  if(!plan){
    console.error("Plan not found for subscription:", order.planId);
...
if (event.event === "invoice.paid") {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.subscription.update({
      ...
      await tx.mandate.updateMany({
        where: { mandateId: subscription.id },
