...
export async function POST(
  ...
  const issuerId = body?.issuedById; // optional
  const participantId = body?.participantId; // optional

  const requesterId = session.user.id;

  // if manual → use participantId
  // else → fallback to existing flow
  const userId = participantId ?? requesterId;
  const paramsContext = await params;
  const programId = paramsContext.id;
  ...
  try {
    ...
    const issuedByIdFinal =
      issuerId && issuerId === program.createdBy
        ? issuerId
    ...
    const signatureDrawn =
      sig?.type === "DRAWN" ? (sig.imageUrl ?? undefined) : undefined;
