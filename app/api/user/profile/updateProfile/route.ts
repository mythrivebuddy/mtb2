...
import { calculateRequiredProfileCompletion } from "@/utils/calculateProfileCompletion";
...
export async function PUT(req: Request) {
  try {
    ...
    const updateData = {
      ...
      yearsOfExperience:
        bodyData.yearsOfExperience !== undefined &&
        bodyData.yearsOfExperience !== null
          ? Number(bodyData.yearsOfExperience)
          : undefined,
    ...
    let gettingStartedStatus = user.gettingStartedStatus;
    if (profile.isProfileComplete !== isCurrentlyComplete || needsJpReward) {
      // Only assign points if they haven't gotten them yet
      if (needsJpReward) {
        assignJp(user, ActivityType.BUSINESSPROFILE_COMPLETE);
      }

      // Update the database with the correct boolean and reward status
      await prisma.userBusinessProfile.update({
        where: { userId },
        data: {
          isProfileComplete: isCurrentlyComplete,
          ...(needsJpReward && { profileJpRewarded: true }),
        },
      });

      if (
        isCurrentlyComplete &&
        user.gettingStartedStatus !== GettingStartedStatus.COMPLETED
      ) {
