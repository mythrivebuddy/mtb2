...
export function GroupCard({ group }: GroupCardProps) {
  ...
  return (
    <Link href={`/dashboard/accountability?groupId=${group.id}`}>
      <div className="block bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm hover:shadow-lg hover:border-blue-400 border border-transparent transition-all duration-300 cursor-pointer transform hover:scale-[1.02]">
            ...
            <div className="flex -space-x-3 items-center flex-shrink-0">
              {displayMembers.map((member, index) => (
                <img
                  ...
                  src={
                    member.user.image ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(member.user.name)}&background=random&color=fff`
                  }
              ...
              {remainingMembers > 0 && (
                <div
                  className="h-10 w-10 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center text-xs font-medium border-2 border-white"
                  style={{ zIndex: 0 }}
                >
                  +{remainingMembers}
                </div>
