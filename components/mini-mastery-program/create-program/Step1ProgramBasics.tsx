...
//     try {
//       // ── Get current session token so Supabase RLS recognises the user ────
//       const { data: sessionData } = await supabase.auth.getSession();
//       if (!sessionData?.session?.access_token) {
//         // No Supabase session — upload via our own API proxy instead
...
//       // ── Direct Supabase upload with auth token ────────────────────────────
...
//       setValue("thumbnailUrl", urlData.publicUrl, { shouldValidate: true });
//       persistToStorage({ ...allValues, thumbnailUrl: urlData.publicUrl } as Partial<Step1Data>);
//       setUploadState("success");
//     } catch (err) {
