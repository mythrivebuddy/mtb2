...
export default function Announcement() {
  ...
  const handleToggleActive = async (announcement: AnnouncementType) => {
    if (isExpired(announcement.expireAt)) return;
    setActiveButtonLoading(true);
    try {
      const { data } = await axios.patch(
        `/api/admin/announcement/${announcement.id}`,
  ...
  const filteredData = data
    ? [...data]
        .filter((a) =>
          audienceFilter === "ALL" ? true : a.audience === audienceFilter
        )
        .sort((a, b) => {
          const aExpired = isExpired(a.expireAt);
          const bExpired = isExpired(b.expireAt);
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg min-h-screen">
      <div className="flex-1 overflow-auto">
        ...
        {/* Add button + Filter dropdown — both on right */}
        <div className="flex justify-end items-center px-4 sm:px-8 pb-4 mb-4 gap-3">
          {/* Add button */}
          <Button
            onClick={() => {
              setEditing(null);
            ...
            className="px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base bg-blue-700 hover:bg-blue-800 whitespace-nowrap"
          >
            Add Announcement
          </Button>
          ...
          <Select
            ...
            onValueChange={(val) => setAudienceFilter(val as AudienceFilter)}
          >
