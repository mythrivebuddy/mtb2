...
/**
 * Converts any YouTube / Loom / Vimeo watch URL into an embeddable src.
 * If it's already an embed URL, returns as-is.
 */
function toEmbedUrl(url: string): string {
  ...
  // Vimeo: https://vimeo.com/ID  →  /video/ID
...
function ProfileMain({ profile }: { profile: BusinessProfile }) {
  ...
  // Resolve profile photo src (handles File object during upload or URL string)
      ...
      <main className="max-w-6xl mx-auto px-6 md:px-10 py-12 space-y-28">
        ...
        <section className="flex flex-col md:flex-row gap-14">
            ...
            <div className="flex items-start gap-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {profile.coachingDomains?.length
                    ? profile.coachingDomains.map((d) => (
                        <span
                          key={d}
          ...
          <div className="w-full md:w-96 bg-white rounded-3xl p-8 shadow-xl border sticky top-10 space-y-6">
            ...
            {profile.priorityContactLink ? (
              <a
                href={profile.priorityContactLink}
                target="_blank"
        ...
        <section className="bg-white border-t-4 border-blue-600 rounded-3xl p-12 shadow-sm space-y-10">
          ...
          {profile.transformation ? (
            <h2 className="text-3xl font-bold text-gray-900 leading-snug max-w-4xl">
              {profile.transformation}
            </h2>
          ) : (
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {profile.typicalResults?.length
              ? profile.typicalResults.map((r, i) => (
              ...
              {profile.servicesOffered.slice(0, 3).map((service, i) => (
                <div
                  ...
                  className={`p-8 rounded-3xl border shadow-sm ${
                    i === 1 ? "border-blue-500 shadow-lg" : "bg-white"
                  }`}
                >
                  {i === 1 && (
                  ...
                  <button className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-xl">
                    View Program
                  </button>
                </div>
              ))}
        ...
        <section className="space-y-6 max-w-3xl mx-auto">
          ...
          <div className="border rounded-2xl bg-white">
            <button
              ...
              className="w-full p-6 text-left font-semibold"
            >
              Session Duration & Format
            </button>
            {open === "session" && (
              <div className="px-6 pb-6 text-sm text-gray-600">
                {profile.sessionDuration || "—"} minutes via{" "}
                {profile.sessionFormat || "Not specified"}.
              </div>
            )}
          </div>
