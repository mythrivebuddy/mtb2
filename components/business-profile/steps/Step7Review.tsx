...
// export default function Step6Trust({ back, submit }) {
...
//     const valid = await trigger(["profilePhoto"])
//     if (valid) submit()
//   }
...
//               JPG, PNG or GIF. Max size 5MB. Recommended square aspect ratio (1:1).
//             </p>
...
//               You're all set! Your profile has everything clients look for.
//             </p>
//           </div>
//         </div>

//         <div className="text-blue-500 text-xl tracking-wide">
//           ★★★★★
...
export default function Step7Trust({
  ...
  // const [triggered, setTriggered] = useState(true) // Track if user has interacted with the photo field

  useEffect(() => {
    const storedData = localStorage.getItem(process.env.LOCALSTORAGE_PROFILE_KEY || "business_profile_draft")
    if (storedData) {
      const data = JSON.parse(storedData)
      if (data.profilePhoto) {
        // setTriggered(false) // Mark as touched when user interacts
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-md max-w-3xl mx-auto">
      ...
      <div className="flex justify-between mt-8">
        ...
        >
          Submit {isLoading && <Loader2 className="ml-2 h-4 w-4 my-1 animate-spin" />}
        </button>
      </div>
    </div>
  )
