...
export default function Step6SessionAvailability({ next, back }: Props) {
  const {
    register,
    watch,
    setValue,
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow">
      ...
      <div className="mb-6">
        ...
        <div
          className={`flex flex-wrap gap-2 ${showError("languages") ? "border border-red-500 p-3 rounded-xl" : ""}`}
        >
          {LANGUAGES.map((lang) => {
          ...
          {(["INR", "USD"] as const).map((currency) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() =>
                  setValue("preferredCurrency", currency, {
                    shouldValidate: true,
                    shouldTouch: true,
                    shouldDirty: true,
                  })
